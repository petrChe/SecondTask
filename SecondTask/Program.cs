...
namespace SecondTask
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            CarOwner carOwner = new CarOwner("fedotov", "bmw", "KZ222EPA02");
            if(carOwner.carNumberValidation(carOwner))
            {
                Console.WriteLine("Regulars are working!");
            }
            else
            {
                Console.WriteLine("Regulars are not  working!");
            }
            
            foreach(string data in CarOwner.GetInfoByBrand("Nissan"))
            {
